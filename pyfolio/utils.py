...
def _date_tickformat(fig, fmt=r'%m月%d日<br>%Y年', row=1, col=1):
    # fig.update_layout(xaxis_tickformat=fmt)
    fig.update_xaxes(tickformat=fmt, row=row, col=col)


def configure_legend(fig, autofmt_xdate=True, rotation=30):
    """
    Format legend for perf attribution plots:
    - put legend to the right of plot instead of overlapping with it
    - make legend order match up with graph lines
